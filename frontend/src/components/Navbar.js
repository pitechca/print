...
const Navbar = () => {
  ...
  return (
    ...
    style={{marginTop: -95+'px'}}>
      <div className="w-full px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          {/* Left container with hamburger menu on mobile and logo */}
          <div className="flex items-center h-full">
            {/* Hamburger Menu - hidden on admin dashboard and moved to left */}
            {!isAdminDashboard && (
              <button
                onClick={toggleMenu}
                className="md:hidden p-1 rounded-md focus:outline-none hover:bg-gray-700 transition-all duration-200 mr-2"
                style={{ 
                  boxShadow: 'none', 
                  border: 'none', 
            ...
            {/* Logo - now directly after hamburger menu with no space on mobile */}
          ...
          {/* Right container with user actions and cart */}
          <div className="flex items-center space-x-4">
            ...
            {/* Cart button - now on the right side */}
            <Link to="/cart"
             className="relative group p-1 rounded-full hover:bg-gray-700 transition-all duration-200"
             onClick={(e) => {
              e.preventDefault();
...
// // working well with the dark design
// // src/components/Navbar.js
// import React, { useState, useEffect } from 'react';
// import { Link, useLocation } from 'react-router-dom';
// import { useAuth } from '../context/AuthContext';
