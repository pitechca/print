...
const ProductCustomization = () => {
  ...
  const handleCustomImage = async (e) => {
    ...
    // file validation
    const validTypes = ['image/jpeg', 'image/png', 'image/gif', 'image/webp', 'image/svg+xml', 'image/heic', 'image/heif'];
          ...
          {!selectedProduct?.inStock && (
            <>
              ...
              {!showNotifyForm ? (
                ...
                <div 
                  className="p-4 border rounded-lg bg-blue-50 transition-all duration-2000 ease-in-out overflow-hidden"
                  style={{
                    maxHeight: showNotifyForm ? '500px' : '0px',
                    opacity: showNotifyForm ? 1 : 0,
                    transform: showNotifyForm ? 'translateY(0)' : 'translateY(-100%)',
                    marginTop: '0.5rem'
                  }}
                >
                  <h4 className="text-xl font-semibold text-blue-800 mb-2">Get Notified</h4>
                  ...
                  <div className="space-y-2">
                    ...
                    <button
                      onClick={(e) => {
                        e.preventDefault();
                        // First trigger the animation by setting a state
                        setFormClosing(true);
                        // Then after animation completes, handle the actual submission
                        setTimeout(() => {
                          handleRemindMe();
              ...
              <div className="flex flex-col sm:flex-row justify-center gap-4 m-0 p-0">
                  ...
                  onClick={() => {
                    ...
                    if (selectedProduct?.images?.length > 0) {
                      // In reorderedImages, the first image from original array is at the end
                      setSelectedProductImage(0);
                      loadProductImage(reorderedImages[0].data);
                    }
                  }}
                  >
            ...
            {designMode === "upload" && (
              <div className="mt-4">
                ...
                <input
                  ...
                  className="w-full"
                />
                {/*Display uploaded image info for "Upload Your Design" */}
